...
Yes. Haven supports webhooks with a REST API and voice gateway: bots can send or bulk-delete messages, queue dynamic voice audio, inspect voice presence, join calls as WebRTC peers, play soundboard sounds, register custom slash commands, and receive HMAC-signed callbacks. Set up webhooks in **Settings → Server Admin Settings → Bots**. See the [Bot Developer Guide](GUIDE.md#-bot--webhook-developer-guide) for full API docs.
