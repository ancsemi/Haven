...
function createHarness({ senderId = 1, sharerId = 1, offerWindows = new Map() } = {}) {
  ...
  const room = new Map([
    [1, { id: 1, socketId: 'socket-1', nativeScreenVersion: 1 }],
    [2, { id: 2, socketId: 'socket-2', nativeScreenVersion: 1 }],
  ]);
  const io = {
    to(socketId) {
...
function createLifecycleHarness({
  ...
  const state = {
    ...
    voiceUsers: new Map([['a1b2c3d4', new Map([
      [1, { id: 1, username: 'User 1', socketId: ownerSocketId, nativeScreenVersion: 1 }],
      [2, { id: 2, username: 'User 2', socketId: 'socket-2', nativeScreenVersion: 1 }],
    ])]]),
...
test('a native sharer revalidates incompatible viewers on fast rejoin', () => {
  ...
  harness.handlers.get('voice-rejoin')({ code: 'a1b2c3d4', nativeScreenVersion: 1 });
